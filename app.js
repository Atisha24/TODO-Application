    ...
    case hasPriorityAndStatusProperties(request.query):
      if (priority === "HIGH" || priority === "MEDIUM" || priority === "LOW") {
        if (
          ...
          response.send(
            data.map((todoItem) => {
              convertTodoDBObjectToResponseObject(todoItem);
            })
          );
        } else {
          response.status(400);
...
app.get("/todos/:todoId/", async (request, response) => {
  ...
  response.send(convertTodoDBObjectToResponseObject(todoData));
});
...
app.get("/agenda/", async (request, response) => {
  ...
  if (isMatch(date, "yyyy-MM-dd")) {
    ...
    const responseResult = await db.all(getTodoDateQuery);
    response.send(
      responseResult((todoItem) => {
        convertTodoDBObjectToResponseObject(todoItem);
      })
    );
  } else {
